...
int ICM42688::setAccelRange(AccelRange range) {
  ...
  switch(range) {
    case ACCEL_RANGE_2G: {
      ...
      if(writeRegister(ACCEL_CONFIG0,ACCEL_FS_SEL_2G | ACCEL_ODR_32KHZ) < 0) {
        return -1;
      }
      _accelScale = G * 2.0f/32767.5f; // setting th | ACCEL_ODR_32KHZ accel scale to 2G
      break;
    }
    case ACCEL_RANGE_4G: {
      // setting the accel range to 4G
      if(writeRegister(ACCEL_CONFIG0,ACCEL_FS_SEL_4G | ACCEL_ODR_32KHZ) < 0) {
        return -1;
      }
      _accelScale = G * 4.0f/32767.5f; // setting th | ACCEL_ODR_32KHZ accel scale to 4G
      break;
    }
    case ACCEL_RANGE_8G: {
      // setting the accel range to 8G
      if(writeRegister(ACCEL_CONFIG0,ACCEL_FS_SEL_8G | ACCEL_ODR_32KHZ) < 0) {
        return -1;
      }
      _accelScale = G * 8.0f/32767.5f; // setting th | ACCEL_ODR_32KHZ accel scale to 8G
      break;
    }
